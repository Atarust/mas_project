 ...
 */
public final class MyWarehouseExample {
	 ...
	 * 
	 * @param testing
	 *            If <code>true</code> the example will run in testing mode,
	...
	public static void run(boolean testing) {
		...
		if (testing) {
			viewBuilder = viewBuilder.withAutoPlay().withAutoClose().withSimulatorEndTime(TEST_END_TIME)
					.withTitleAppendix("TESTING").withSpeedUp(TEST_SPEED_UP);
		} else {
		...
		final Simulator sim = Simulator.builder().addModel(RoadModelBuilders.dynamicGraph(GraphCreator.createGraph())
				.withCollisionAvoidance().withDistanceUnit(SI.METER).withVehicleLength(VEHICLE_LENGTH))
				.addModel(viewBuilder).build();

		for (int i = 0; i < NUM_AGVS; i++) {
			sim.register(new MyAGVAgent(sim.getRandomGenerator()));
		}
	...
	static class GraphCreator {
		...
		static ImmutableTable<Integer, Integer, Point> createMatrix(int cols, int rows, Point offset) {
			...
			for (int c = 0; c < cols; c++) {
				for (int r = 0; r < rows; r++) {
					builder.put(r, c, new Point(offset.x + c * VEHICLE_LENGTH * 2, offset.y + r * VEHICLE_LENGTH * 2));
				}
			}
		...
		static ListenableGraph<LengthData> createSimpleGraph() {
			...
			final Table<Integer, Integer, Point> matrix = createMatrix(8, 6, new Point(0, 0));
			...
			Graphs.addPath(g, Lists.reverse(newArrayList(matrix.row(matrix.rowKeySet().size() - 1).values())));
		...
		static ListenableGraph<LengthData> createGraph() {
			...
			final Table<Integer, Integer, Point> leftMatrix = createMatrix(5, 10, new Point(0, 0));
			...
			final Table<Integer, Integer, Point> rightMatrix = createMatrix(10, 7, new Point(30, 6));
			...
			Graphs.addPath(g, rightMatrix.get(RIGHT_CENTER_U_ROW, RIGHT_COL),
					leftMatrix.get(LEFT_CENTER_U_ROW, LEFT_COL));
			Graphs.addPath(g, leftMatrix.get(LEFT_CENTER_L_ROW, LEFT_COL),
